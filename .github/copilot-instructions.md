...
| `AbstractAppSettings` (`OzzMarkdown.Core.Models`) | Abstract base for app settings: `UiCulture`, `SelectedTheme`, `RecentFiles`, and JSON persistence via `Save`. `AddRecentFile(filePath, maxCount)` adds a path to the front of the recent-files list, removes case-insensitive duplicates, and trims it to the requested size. Subclasses provide `GetSettingsFolderName()` and app-specific settings file naming/singleton logic (see `OzzMarkdown.WPF.Models.AppSettings`). |
...
- `AppSettings` (`Models/AppSettings.cs`): Singleton (thread-safe lazy init) extending `AbstractAppSettings`; persists settings, including recent-file history and `MainWindowPosition`, as JSON to `%AppData%/OzzMarkdown/wpfsettings.json`. `MainWindow.xaml` binds the toolbar's Recent Files ComboBox to `MainViewModel.RecentFiles` and `SelectedRecentFile`. Call `GetAppSettings()` to read, `Save()` to write.
